...
model {
	...
	for(i in 1:N)
	{
		y[i] ~ normal( (a*((R[i]*F[z[i]])^bF + bT1*T_t[i] + bT2*T_t[i]^2)) * exp(-(t[i] - mu)^2 / ( 2 * sigma[Index[i]]^2)), obs_error[i]);
	}
}
generated quantities {
  ...
  for (i in 1:N){
    log_lik[i] = normal_lpdf(y[i] |(a*((R[i]*F[z[i]])^bF + bT1*T_t[i] + bT2*T_t[i]^2))  * exp(-(t[i] - mu)^2 / ( 2 * sigma[Index[i]]^2)), obs_error[i]);
 } 
  // Generate posterior predictive checks
  for (i in 1:N) {
y_rep[i] = normal_rng( (a*((R[i]*F[z[i]])^bF + bT1*T_t[i] + bT2*T_t[i]^2))  * exp(-(t[i] - mu)^2 / ( 2 * sigma[Index[i]]^2)), obs_error[i]);
  }

...
model {
  ...
  sigma_obs ~ normal(pr_sigma_obs,pr_sigma_obs_sd);
	...
	for(i in 1:N)
	{
      y[i] ~ normal( A + B[Index[i]] * exp(-t[i] * C), obs_error[i]);
	  }
}
 generated quantities {
   ...
   for (i in 1:N){
     log_lik[i] = normal_lpdf(y[i] | A + B[Index[i]] * exp(-t[i] * C), obs_error[i]);
  }   
   // Generate posterior predictive checks
   for (i in 1:N) {
 y_rep[i] = normal_rng(A + B[Index[i]] * exp(-t[i] * C), obs_error[i]);
   } 
 }

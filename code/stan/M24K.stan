...
model {
  ...
  sigma_obs ~ normal(pr_sigma_obs,pr_sigma_obs_sd);
	for(i in 1:N)
	{
  y[i] ~ normal((A[Index[i]]*(T_t[i]^bTA)*(R[i]*F[z[i]])^bF) + (B*(bTB1*T_t[i]+bTB2*T_t[i]^2)*(R[i]*F[z[i]])^bF) * exp(-t[i] * C) , obs_error[i]);
    }
}
generated quantities {
  ...
  for (i in 1:N){
    log_lik[i] = normal_lpdf(y[i] | (A[Index[i]]*(T_t[i]^bTA)*(R[i]*F[z[i]])^bF) + (B*(bTB1*T_t[i]+bTB2*T_t[i]^2)*(R[i]*F[z[i]])^bF)  * exp(-t[i] * C) , obs_error[i]);
 }
  
  // Generate posterior predictive checks
  for (i in 1:N) {
	y_rep[i] = normal_rng((A[Index[i]]*(T_t[i]^bTA)*(R[i]*F[z[i]])^bF) + (B*(bTB1*T_t[i]+bTB2*T_t[i]^2)*(R[i]*F[z[i]])^bF) * exp(-t[i] * C) , obs_error[i]);
  }
}
